...
download_checksums() {
    ...
    (
        curl --retry 15 --retry-delay 10 --retry-all-errors --retry-connrefused -sL "$CHECKSUM_URL" -o "$CHECKSUM_FILE" &
...
download_gripmock() {
    ...
    (
        curl --retry 15 --retry-delay 10 --retry-all-errors -sL "$DOWNLOAD_URL" -o "$DOWNLOAD_FILE" &
