...
get_latest_version() {
    ...
    LATEST_RELEASE=$(curl --retry 10 -s https://api.github.com/repos/bavix/gripmock/releases/latest)
...
download_checksums() {
    ...
    (
        curl --retry 10 -sL "$CHECKSUM_URL" -o "$CHECKSUM_FILE" &
...
download_gripmock() {
    ...
    (
        curl --retry 10 -sL "$DOWNLOAD_URL" -o "$DOWNLOAD_FILE" &
