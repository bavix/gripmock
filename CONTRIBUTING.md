...
### Gateway testing (ConnectRPC and gRPC-Web)

grpctestify speaks all three wire protocols, so the same `.gctf` files must pass
over each one. Point it at the gateway port and switch the protocol:
