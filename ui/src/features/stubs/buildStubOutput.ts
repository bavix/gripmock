...
export function buildOutput(f: StubOutputFields, outMode: 'data' | 'stream'): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  if (outMode === 'data') { const d = parse(f.outputData); if (d != null) out.data = d; }
  if (outMode === 'stream') { const s = parse(f.outputStream); if (s != null) out.stream = s; }
