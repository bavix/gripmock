...
// This is an exported version of newSearcher for testing purposes.
...
func (br *BidiResult) stubMatchesMessage(stub *Stub, messageData map[string]any) bool {
	...
	if stub.IsBidirectional() {
		// V4 format: use InputsV4 for input matching
		if len(stub.InputsV4) > 0 {
			for _, matcher := range stub.InputsV4 {
				if br.matchV4Matcher(matcher, messageData) {
					return true
				}
			}
		...
		// Legacy format: use Inputs for input matching
	...
	if stub.IsUnary() {
		// V4 format: use InputsV4 for input matching
		if len(stub.InputsV4) > 0 {
			return br.matchV4Matcher(stub.InputsV4[0], messageData)
		}
		// Legacy format: use Input for matching
		return br.matchInputData(stub.Input, messageData)
	}
...
// matchV4Matcher checks if messageData matches the given v4 Matcher.
//

//nolint:gocognit,cyclop
func (br *BidiResult) matchV4Matcher(matcher types.Matcher, messageData map[string]any) bool {
	// Early exit if Matcher is empty
	if len(matcher.Equals) == 0 && len(matcher.Contains) == 0 && len(matcher.Matches) == 0 && len(matcher.Any) == 0 {
		return true
	}

	// Check Equals
	if len(matcher.Equals) > 0 {
		for key, expectedValue := range matcher.Equals {
			if actualValue, exists := br.findValueWithVariations(messageData, key); !exists || !deepEqual(actualValue, expectedValue) {
				return false
			}
		}
	...
	if len(matcher.Contains) > 0 {
		for key, expectedValue := range matcher.Contains {
			actualValue, exists := messageData[key]
			if !exists {
				return false
			}
			// Create minimal map for contains check
			tempMap := map[string]any{key: expectedValue}
			if !contains(tempMap, actualValue, matcher.IgnoreArrayOrder) {
				return false
			}
		}
	}
	...
	if len(matcher.Matches) > 0 {
		for key, expectedValue := range matcher.Matches {
			actualValue, exists := messageData[key]
			if !exists {
				return false
			}
			// Create minimal map for matches check
			tempMap := map[string]any{key: expectedValue}
			if !matches(tempMap, actualValue, matcher.IgnoreArrayOrder) {
				return false
			}
		}
	}
	...
	if len(matcher.Any) > 0 {
		for _, subMatcher := range matcher.Any {
			if br.matchV4Matcher(subMatcher, messageData) {
				return true
			}
		}
...
func (s *searcher) fastMatchInput(queryData map[string]any, stubInput InputData) bool {
	...
	if len(queryData) == 0 {
		return len(stubInput.Equals) == 0 && len(stubInput.Contains) == 0 && len(stubInput.Matches) == 0 && len(stubInput.Any) == 0
	}

	// Check Any matcher (OR logic) - if Any exists, it must match
	if len(stubInput.Any) > 0 {
		for _, anyMatcher := range stubInput.Any {
			if s.fastMatchInput(queryData, anyMatcher) {
				return true
			}
		}
...
func (s *searcher) fastRankInput(queryData map[string]any, stubInput InputData) float64 {
	...
	if len(queryData) == 0 {
		...
		if len(stubInput.Equals) == 0 && len(stubInput.Contains) == 0 && len(stubInput.Matches) == 0 && len(stubInput.Any) == 0 {
	...
	if len(stubInput.Any) > 0 {
		...
		for _, anyMatcher := range stubInput.Any {
			anyRank = max(anyRank, s.fastRankInput(queryData, anyMatcher))
		}

		// If any condition matches, return its rank
		if anyRank > 0 {
			return anyRank
		}
	}
