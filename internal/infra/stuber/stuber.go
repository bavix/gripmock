...
// titleEnglishNoLower uppercases only the first rune without lowercasing the rest.
func titleEnglishNoLower(s string) string {
	if s == "" {
		return s
	}
...
func (b *Budgerigar) FindByQuery(query Query) (*Result, error) {
	if b.toggles.Has(MethodTitle) {
		query.Method = titleEnglishNoLower(query.Method)
	}
...
// - method: The method name to search for.
//
// Returns:
// - []*Stub: All matching Stub values, sorted by priority in descending order.
// - error: An error if the search fails.
