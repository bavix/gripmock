...
//nolint:funlen
func TestProcessStubs(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			stubs := make([]*Stub, tc.stubSize)
			for i := range tc.stubSize {
				stubs[i] = &Stub{
					ID:       uuid.New(),
					Service:  "test.service",
					Method:   "TestMethod",
					Priority: i % tc.priority,
					Input: InputData{
			...
			query := QueryV2{
				Service: "test.service",
				Method:  "TestMethod",
				Input:   []map[string]any{{"id": tc.queryID}},
			}

			result, err := searcher.processStubs(query, stubs)
			require.NoError(t, err)
			require.NotNil(t, result.Found())
			require.Equal(t, tc.expected, result.Found().Output.Data["result"])
		})
	}
}
