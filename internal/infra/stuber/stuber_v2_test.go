...
func TestFieldNameVariations(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			stubFieldName:  "user_name",
			queryFieldName: "userName",
		},
		{
			name:           "camelCase to snake_case",
			stubFieldName:  "userName",
			queryFieldName: "user_name",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			stub := &stuber.Stub{
				...
				Method:  "Test",
				Input: stuber.InputData{
					Equals: map[string]any{tc.stubFieldName: "john"},
				},
				Output: stuber.Output{
			...
			// Test with different field name (should match)
			stubResult, err := result.Next(map[string]any{tc.queryFieldName: "john"})
			require.NoError(t, err)
			require.NotNil(t, stubResult)
			require.Equal(t, "Hello John!", stubResult.Output.Data["response"])

			// Test with exact field name
			stubResult2, err := result.Next(map[string]any{tc.stubFieldName: "john"})
			require.NoError(t, err)
