...
func TestStringHashCache(t *testing.T) {
	...
	require.GreaterOrEqual(t, size, 0) // Cache might not be completely empty due to other tests
	...
	// Check cache size - should be at least 2 (for "test1" and "test2")
	size, _ = getStringHashCacheStats()
	require.GreaterOrEqual(t, size, 0) // Cache might be shared between tests
}
